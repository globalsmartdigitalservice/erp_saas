...
Está apagado por defecto (`TENANCY_POR_HEADER = False`) y encendido en
...
HEADER = "HTTP_X_IDIOMA"


class IdiomaDesdeHeaderMiddleware:
    def __init__(self, get_response):
        if not getattr(settings, "TENANCY_POR_HEADER", False):
            raise MiddlewareNotUsed

        self.get_response = get_response

    def __call__(self, request):
        crudo = request.META.get(HEADER)
